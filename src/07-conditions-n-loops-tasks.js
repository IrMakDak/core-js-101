...
function findFirstSingleChar(str) {
  const result = {};
  const mySet = Array.from(new Set(str));
  mySet.forEach((num) => {
    result[num] = 0;
  });
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] in pairs) {
      ...
      if (!lastOpening || pairs[lastOpening] !== str[i]) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const pathLength = splittedPathes[0].length;
  let commonPath = '';
  for (let i = 0; i < pathLength; i += 1) {
    const pathPart = splittedPathes[0][i];
    if (splittedPathes.some((path) => path[i] !== pathPart)) {
      break;
    }
    commonPath += `${pathPart}/`;
  }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    ...
    if (position[i][0] === position[i][1]) {
      if (position[i][0] !== undefined && position[i][1] === position[i][2]) {
        return position[i][0];
      }
    }
    // check columns
